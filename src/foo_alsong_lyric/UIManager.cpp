...
UIManager::UIManager(UIPreference *Setting, pfc::string8 *Script) : m_Setting(Setting), m_Script(Script)
{
	...
	SquirrelObject DrawScript = SquirrelVM::CompileBuffer(TEXT("function Draw(canvas, lines) { local font = UIFont(\"맑은 고딕\", 10, WndSetting.GetFontColor()); local h = 0; foreach(i,v in lines) {local sz = canvas.EstimateText(font, v); h += sz.height} foreach(i, v in lines){ canvas.DrawText(font, v);}}"));
