...
enum ScriptVarType {
    ...
    VAR_TYPE_STD_STRING,
#endif
    VAR_TYPE_INSTANCE_PTR,  // for pointer data member
};
...
// While in template functions, the const-ness may be casted away, then we need
// this function.
template<>
struct TypeInfo<SQChar *> {
  const SQChar * typeName;
  TypeInfo() : typeName(_SC("SQChar *")) {}
  enum {TypeID=VAR_TYPE_CONST_STRING,Size=sizeof(const SQChar *),TypeMask='s', IsInstance=0};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
...
template<>
struct TypeInfo<SQOtherChar *> {
  const SQChar * typeName;
  TypeInfo() : typeName(_SC("SQOtherChar *")) {}
  enum {TypeID=VAR_TYPE_CONST_STRING,Size=sizeof(const SQOtherChar *),TypeMask='s', IsInstance=0};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
template<>
...
// for const T* data members
template<typename T>
void RegisterInstanceVariable(SquirrelObject & so,ClassTypeBase* classType, const T** var,const SQChar * scriptVarName,VarAccessType access=VAR_ACCESS_READ_WRITE) {
  RegisterInstanceVariable(so, classType, const_cast<T**>(var), scriptVarName, access);
} // RegisterInstanceVariable
...
inline SquirrelObject NewClassCopy(HSQUIRRELVM v, const SQChar * className,const T & classToCopy) {
  if (CreateCopyInstance(v, className,classToCopy)) {
    ...
    throw SquirrelError(_SC("NewClassCopy(): could not create instance of: "),className);
  } // if
...
inline RT GetRet(TypeWrapper<RT>, HSQUIRRELVM v, int idx) {
	SquirrelObject& sq_ret_ref = SquirrelVM::GetRetRef();
	static typename Temporary<RT>::type st_ret;
    st_ret = Get(TypeWrapper<RT>(), v, idx);
	sq_ret_ref.AttachToStackObject(idx);
	sq_pop(v,2); // restore stack after function call.
	return st_ret;
}

// Specialization for SquirrelObject return value
template<>
inline SquirrelObject GetRet(TypeWrapper<SquirrelObject>,HSQUIRRELVM v,int idx) {
	SquirrelObject& sq_ret_ref = SquirrelVM::GetRetRef();
	sq_ret_ref = Get(TypeWrapper<SquirrelObject>(),v,idx);
	sq_pop(v,2); // restore stack after function call.
	return sq_ret_ref;
}

// No reference hold for primitive types
template<typename RT>
inline RT GetRetNoReference(TypeWrapper<RT>, HSQUIRRELVM v, int idx) {
	typename Temporary<RT>::type ret;
    ret = Get(TypeWrapper<RT>(), v, idx);
	sq_pop(v,2); // restore stack after function call.
	return ret;
}

#define SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(t) \
  template<> inline t GetRet(TypeWrapper<t>, HSQUIRRELVM v, int idx) { \
    return GetRetNoReference(TypeWrapper<t>(), v, idx); \
  }
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(int);
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(unsigned);
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(FLOAT);
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(bool);
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(short);
SQPLUS_SPECIALIZE_GETRET_BY_NOREFERENCE(char);
...
#define DECLARE_INSTANCE_TYPEINFO_(TYPE,NAME) \
  ...
  template<> \
    struct TypeInfo<TYPE*> { \
      const SQChar * typeName; \
       TypeInfo() : typeName( _SC(#NAME)) {} \
       enum {TypeID=VAR_TYPE_INSTANCE_PTR,Size=sizeof(TYPE*),TypeMask='x', IsInstance=1}; \
       operator ScriptVarType() { return ScriptVarType(TypeID); } \
  };
...
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace SqPlus { \
  ...
  template<> inline void Push(HSQUIRRELVM v,TYPE * value) { \
	...
	throw SquirrelError( _SC("Push(): could not create INSTANCE of: "), _SC(#NAME) ); } \
  template<> inline void Push(HSQUIRRELVM v,TYPE & value) { \
    if (!CreateCopyInstance(v,GetTypeName(value),value)) \
	throw SquirrelError( _SC("Push(): could not create INSTANCE copy of: "), _SC(#NAME) ); } \
} // nameSpace SqPlus
...
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace SqPlus { \
  DECLARE_INSTANCE_TYPEINFO_(TYPE,NAME) \
  template<> inline void Push(HSQUIRRELVM v,TYPE * value)                 { if (!CreateNativeClassInstance(v,GetTypeName(*value),value,0)) throw SquirrelError( _SC( "Push(): could not create INSTANCE (check registration name): "#NAME) ); } \
  template<> inline void Push(HSQUIRRELVM v,TYPE & value)                 { if (!CreateCopyInstance(v,GetTypeName(value),value)) throw SquirrelError( _SC( "Push(): could not create INSTANCE copy (check registration name): "#NAME) ); } \
  template<> inline bool	Match(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE,false>(v,idx) != NULL; } \
  template<> inline bool	Match(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE,false>(v,idx) != NULL; } \
  template<> inline TYPE & Get(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return *GetInstance<TYPE,true>(v,idx); } \
  template<> inline TYPE * Get(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE,true>(v,idx); } \
...
#define DECLARE_INSTANCE_TYPE_NOCOPY(TYPE) \
  DECLARE_INSTANCE_TYPE_NAME_(TYPE,TYPE) \
  DECLARE_NONCOPY_TYPE(TYPE)
...
#define SQ_ERR_BUF_LEN 256
extern SQChar g_sq_err_buf[SQ_ERR_BUF_LEN];

#ifndef SQ_SKIP_ARG_ASSERT
  #define sq_argassert(arg,_index_) \
  if (!Match(TypeWrapper<P##arg>(),v,_index_)){ \
    scsprintf(g_sq_err_buf,_SC("Incorrect function argument - Expecting (arg#%d): %s"),arg,TypeInfo<P##arg>().typeName); \
	return sq_throwerror(v,g_sq_err_buf); }
#else
  #define sq_argassert(arg,_index_)
#endif
...
struct DirectCallInstanceFuncPicker {
  ...
  DirectCallInstanceFuncPicker(HSQUIRRELVM v) {
	...
	if( instance ){
      SquirrelObject so(sa.GetObjectHandle(1)); // 'this'
      ClassTypeBase *instType; so.GetInstanceTypeTag((SQUserPointer*)&instType);
      ClassTypeBase *calleeType = ClassType<Callee>::type();
      if (instType!=calleeType && instType && instType->MayHaveOffset() ) {
...
#include "SqPlusConst.h"
  ...
  SQClassDefBase & staticVar( const SQChar *name, int ival ){ return scriptVar(name,ival,SQTrue); }

  SQClassDefBase & scriptVar( const SQChar* name, double fval, SQBool static_var=SQFalse ) {
    HSQUIRRELVM v = SquirrelVM::GetVMPtr();
    sq_pushobject(v,newClass.GetObjectHandle());
    sq_pushstring(v,name,-1);
    sq_pushfloat(v,(SQFloat)fval);
    sq_newslot(v,-3,static_var);
    sq_pop(v,1);
    return *this;
  }
  SQClassDefBase & staticVar( const SQChar *name, double fval ){ return scriptVar(name,fval,SQTrue); }
  ...
  SQClassDefBase & staticVar( const SQChar *name, const SQChar* sval ){ return scriptVar(name,sval,SQTrue); }
