...
SquirrelObject  SquirrelVM::_ret_ref;
...
SQSharedStateNode::~SQSharedStateNode() {
    if(m_ps){ 
        //printf( "SQSharedStateNode::~SQSharedStateNode\n" );
        sq_delete(m_ps,SQSharedState); 
    }
    delete m_nxt;
}
...
SquirrelError::SquirrelError() : desc(desc_buf) 
{
    const SQChar *s;
    sq_getlasterror(SquirrelVM::_VM);
    sq_getstring(SquirrelVM::_VM,-1,&s);
	if( !s )
		s = _SC("<unknown error (sq_getlasterror failed)>");
	int sl = (int)scstrlen(s);
	if( sl>SQ_MAX_ERR_LEN-1 )
		sl = SQ_MAX_ERR_LEN-1;
	scstrncpy(desc_buf,s,sl);
	desc_buf[sl] = 0;
}
...
bool SquirrelVM::Init( HSQUIRRELVM v ){
    //printf( "SquirrelVM::Init - %p (old:%p)\n", v, _VM );
    if(v && v==_VM) {
        return true;
    }
...
void SquirrelVM::Release() {
    ...
    // Drop our ref on any previous return value
    _ret_ref.Reset();
    
    // Release our ref on VM - if we should
    if( !_no_vm_ref ){
        // Release our hold on the VM
        _vm.Reset();
    }
...
void SquirrelVM::Shutdown() {
    ...
    // The reference is not valid here, so just undo it
    sq_resetobject( &(HSQOBJECT&)_vm );
    
    // Find the shared state node stored in the linked list
    // unlink it and destroy the node.
    SQSharedStateNode** ppn = &g_sqss_fst;
    while( *ppn && (*ppn)->m_ps!=_ss(_VM) )
        ppn = &((*ppn)->m_nxt);
    if( *ppn ){
        SQSharedStateNode *pn = *ppn;
        *ppn = pn->m_nxt;
        pn->m_nxt = NULL;
        pn->m_ps = NULL;
        delete pn;
...
void SquirrelVM::AppFinalShutdown(){
    // Destroy whatever resources we still own. 
    // It is safer to manually do this before global destructors 
    // are run.
    Shutdown();
    delete g_sqss_fst;
    g_sqss_fst = NULL;
