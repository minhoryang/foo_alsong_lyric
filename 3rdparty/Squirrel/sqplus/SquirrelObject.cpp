...
void SquirrelObject::WeakrefToStackObject(int idx)
{
	sq_weakref(SquirrelVM::_VM, idx);
    AttachToStackObject(-1);
    sq_pop(SquirrelVM::_VM, 1);
}

void SquirrelObject::Weakref(const SquirrelObject &o)
{
    sq_pushobject(SquirrelVM::_VM, o.GetObjectHandle());
    WeakrefToStackObject(-1);
    sq_pop(SquirrelVM::_VM, 1);
}
...
SQObjectType SquirrelObject::GetType() const
{
	return _o._type;
}
...
HSQOBJECT
SquirrelObject::GetWeakrefVal(void) const
{
    HSQOBJECT t;
    if (GetType() == OT_WEAKREF) {
        sq_pushobject(SquirrelVM::_VM, _o);
        if (SQ_SUCCEEDED(sq_getweakrefval(SquirrelVM::_VM, -1)) &&
            SQ_SUCCEEDED(sq_getstackobj(SquirrelVM::_VM, -1, &t))) {
            sq_pop(SquirrelVM::_VM, 2);
            return t;
        }
    }
...
BOOL SquirrelObject::GetInstanceTypeTag(SQUserPointer * typeTag) {
  BOOL r = GetTypeTag(typeTag);
  if( type(_o)!=OT_INSTANCE ) return FALSE;
  SQInstance *pi = _o._unVal.pInstance;
  SQClass *pc = pi->_class;
  if( pc->_typetag ){
    *typeTag = (SQUserPointer*)pc->_typetag;
    return TRUE;
  }

  SQClass *pc_der;  // Let the typetag propagate one step to derived
  while( true ){
	pc_der = pc;
	pc = pc->_base;
	if( !pc ) return FALSE;
    if( pc->_typetag ){
      pc_der->_typetag = pc->_typetag;
      *typeTag = (SQUserPointer*)pc->_typetag;
      return TRUE;
    }
  }
} // SquirrelObject::GetInstanceTypeTag
