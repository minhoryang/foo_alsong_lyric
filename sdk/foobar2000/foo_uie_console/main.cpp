...
const t_size maximum_messages = 50;
...
public:
	...
	static void g_on_message_received(const char * ptr, t_size len) //from any thread
	{
		...
		if (g_messages.add_item(t_message(buffer.get_ptr(), buffer.get_length())) == maximum_messages)
...
private:
	static HFONT g_font;
	static pfc::list_t<t_message> g_messages;
...
pfc::list_t<console_window::t_message> console_window::g_messages;
...
console_window::console_window() : wnd_edit(NULL), m_editproc(NULL)
{
}
...
/**
* This is the unique GUID identifying out panel. You should not re-use this one
